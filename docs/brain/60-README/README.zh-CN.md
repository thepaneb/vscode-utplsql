...
[[README (extensão)|English]] · **中文(简体)** · [[README.zh-TW|中文(繁體)]] · [[README.ja|日本語]] · [[README.ko|한국어]] · [[README.es|Español]] · [[README.fr|Français]] · [[README.pt-BR|Português]] · [[README.it|Italiano]] · [[README.ro|Română]] · [[README.de|Deutsch]] · [[README.ru|Русский]] · [[README.pl|Polski]] · [[README.uk|Українська]] · [[README.cs|Čeština]] · [[README.bg|Български]] · [[README.sr|Српски]] · [[README.tr|Türkçe]] · [[README.el|Ελληνικά]] · [[README.hu|Magyar]] · [[README.id|Bahasa Indonesia]] · [[README.vi|Tiếng Việt]] · [[README.th|ไทย]] · [[README.en-GB|English (UK)]]
...
| `utplsql.coverage.schemes` | `[]` | 覆盖的 schema（`a_coverage_schemes`）。留空 = 连接用户（或 `utplsql.coverageOwner`）。 |
| `utplsql.coverage.includeObjects` | `[]` | 要纳入覆盖的对象，格式为 `OWNER.NAME`（例如 `["APP.MY_PKG"]`）。适用于仅通过动态方式到达的对象。 |
| `utplsql.coverage.excludeObjects` | `[]` | 要从覆盖中排除的对象，格式为 `OWNER.NAME`（例如 `["UT3.UT_COVERAGE"]`）。 |
| `utplsql.coverage.includeSchemaExpr` | `""` | 要纳入覆盖的 schema 正则（例如 `^APP$`）。 |
| `utplsql.coverage.includeObjectExpr` | `""` | 要纳入覆盖的对象正则。 |
| `utplsql.coverage.excludeSchemaExpr` | `""` | 要从覆盖中排除的 schema 正则。 |
| `utplsql.coverage.excludeObjectExpr` | `""` | 要从覆盖中排除的对象正则（例如 utPLSQL 框架用 `^UT_`）。 |
...
| `utplsql.language` | `auto` | 运行时消息的语言。`auto` 跟随 VSCode（pt、zh-tw/zh-hk、zh、es、ja、de、fr、it、ko、ru、tr、pl、cs、hu、bg、el、id、ro、sr、th、uk、vi、en-gb；否则为 en）。覆盖 **24 种语言环境**。 |
...
| `-- %tags(fast, critical)` | 测试标签；使用 `utplsql.tags` 设置筛选运行（例如 `fast & !integration`） |
...
扩展始终包含**两个**默认 reporter：
`ut_documentation_reporter`（stdout）和
`ut_junit_reporter`（结果 → Test Explorer）。
`ut_coverage_cobertura_reporter` 仅在带覆盖率运行时添加。

**动态验证** — 在运行覆盖率之前，扩展通过 `TABLE(ut_runner.get_reporters_list())` 查询
...
| 需要诊断 | 不清楚扩展内部正在做什么 | 在启动 VSCode 前设置 `UTPLSQL_DEBUG=1`，以在扩展主机控制台中启用可选诊断日志（连接/发现/覆盖率失败的上下文） |
