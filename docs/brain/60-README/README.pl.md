...
[[README (extensão)|English]] · [[README.zh-CN|中文(简体)]] · [[README.zh-TW|中文(繁體)]] · [[README.ja|日本語]] · [[README.ko|한국어]] · [[README.es|Español]] · [[README.fr|Français]] · [[README.pt-BR|Português]] · [[README.it|Italiano]] · [[README.ro|Română]] · [[README.de|Deutsch]] · [[README.ru|Русский]] · **Polski** · [[README.uk|Українська]] · [[README.cs|Čeština]] · [[README.bg|Български]] · [[README.sr|Српски]] · [[README.tr|Türkçe]] · [[README.el|Ελληνικά]] · [[README.hu|Magyar]] · [[README.id|Bahasa Indonesia]] · [[README.vi|Tiếng Việt]] · [[README.th|ไทย]] · [[README.en-GB|English (UK)]]
...
| `utplsql.coverage.schemes` | `[]` | Pokrywane schematy (`a_coverage_schemes`). Puste = użytkownik połączenia (lub `utplsql.coverageOwner`). |
| `utplsql.coverage.includeObjects` | `[]` | Obiekty do uwzględnienia w pokryciu, jako `OWNER.NAME` (np. `["APP.MY_PKG"]`). Przydatne dla obiektów osiąganych tylko dynamicznie. |
| `utplsql.coverage.excludeObjects` | `[]` | Obiekty do wykluczenia z pokrycia, jako `OWNER.NAME` (np. `["UT3.UT_COVERAGE"]`). |
| `utplsql.coverage.includeSchemaExpr` | `""` | Regex schematów do uwzględnienia w pokryciu (np. `^APP$`). |
| `utplsql.coverage.includeObjectExpr` | `""` | Regex obiektów do uwzględnienia w pokryciu. |
| `utplsql.coverage.excludeSchemaExpr` | `""` | Regex schematów do wykluczenia z pokrycia. |
| `utplsql.coverage.excludeObjectExpr` | `""` | Regex obiektów do wykluczenia z pokrycia (np. `^UT_` dla frameworka utPLSQL). |
...
| `utplsql.oracleClientMode` | `thin` | Tryb sterownika: `thin` (czysty JavaScript, bez natywnego klienta) lub `thick` (używa Oracle Instant Client). Używaj `thick` tylko dla baz wymagających NNE (Native Network Encryption); wymaga `utplsql.oracleClientLibDir` i przeładowania okna. |
...
| `utplsql.debugger.enabled` | `true` | Włącza debugowanie testów PL/SQL (`DBMS_DEBUG`). Wymaga `node-oracledb` + grantów. Skompiluj pakiet docelowy z informacjami debugowania (`PLSQL_OPTIMIZE_LEVEL <= 1`) i nadaj `DEBUG CONNECT SESSION` + `EXECUTE ON SYS.DBMS_DEBUG`. |
...
| `utplsql.debugger.compileOnDebug` | `false` | Kompiluje obiekt z informacjami debugowania (`ALTER … COMPILE DEBUG PLSQL_OPTIMIZE_LEVEL = 1`) przed rozpoczęciem sesji debugowania. |
...
| `utplsql.language` | `auto` | Język komunikatów środowiska uruchomieniowego. `auto` podąża za VSCode (pt, zh-tw/zh-hk, zh, es, ja, de, fr, it, ko, ru, tr, pl, cs, hu, bg, el, id, ro, sr, th, uk, vi, en-gb; w przeciwnym razie en). Obejmuje **24 lokalizacje**. |
...
11. **utPLSQL: Cancel run** — zatrzymuje bieżące wykonanie (`Escape` podczas wykonania).
...
| `-- %tags(fast, critical)` | Tagi testu; filtruj uruchomienie ustawieniem `utplsql.tags` (np. `fast & !integration`) |
...
| `utPLSQL: Cancel run` | Zatrzymuje bieżące wykonanie | — |
| `utPLSQL: Show utPLSQL info` | Informacje o wersji z opcją kopiowania | — |
...
| `utPLSQL: Validate setup` | Wykonuje pełną walidację konfiguracji (połączenie, instalacja UT3) i pokazuje wyniki | — |
...
Rozszerzenie zawsze zawiera **dwa** domyślne reportery:
`ut_documentation_reporter` (stdout) i
`ut_junit_reporter` (wyniki → Test Explorer).
`ut_coverage_cobertura_reporter` jest dodawany **tylko przy uruchomieniu z pokryciem**.

**Dynamiczna walidacja** — przed uruchomieniem z pokryciem rozszerzenie odpytuje
bazę danych przez `TABLE(ut_runner.get_reporters_list())`. Jeśli
...
|---|---|---|
| Suite'y się nie pojawiają | Nie znaleziono bazy danych | Uruchom `utPLSQL: Validate setup`, aby uzyskać diagnostykę |
...
| Błąd kompilacji bez wskazówki | Kod z błędem składni PL/SQL | Pozostaw `utplsql.compilationDiagnostics.enabled` włączone (domyślnie); błędy z `ALL_ERRORS` pojawiają się w panelu "Problemy" po uruchomieniu |
| Błąd połączenia | Nieprawidłowy ciąg lub nieosiągalna baza | Użyj `utPLSQL: Validate setup` |
...
| Potrzebna diagnostyka | Niejasne, co rozszerzenie robi wewnętrznie | Ustaw `UTPLSQL_DEBUG=1` przed uruchomieniem VSCode, aby włączyć logi diagnostyczne (kontekst awarii połączenia/wykrywania/pokrycia) w konsoli Extension Host |
