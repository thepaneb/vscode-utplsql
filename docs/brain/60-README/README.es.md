...
[[README (extensão)|English]] · [[README.zh-CN|中文(简体)]] · [[README.zh-TW|中文(繁體)]] · [[README.ja|日本語]] · [[README.ko|한국어]] · **Español** · [[README.fr|Français]] · [[README.pt-BR|Português]] · [[README.it|Italiano]] · [[README.ro|Română]] · [[README.de|Deutsch]] · [[README.ru|Русский]] · [[README.pl|Polski]] · [[README.uk|Українська]] · [[README.cs|Čeština]] · [[README.bg|Български]] · [[README.sr|Српски]] · [[README.tr|Türkçe]] · [[README.el|Ελληνικά]] · [[README.hu|Magyar]] · [[README.id|Bahasa Indonesia]] · [[README.vi|Tiếng Việt]] · [[README.th|ไทย]] · [[README.en-GB|English (UK)]]
...
**Compatibilidad Oracle / utPLSQL:**

| Oracle | utPLSQL | Notas |
|---|---|---|
| 18c+ | v3.2.x (18c+) / v3.1.x | Recomendado; charset `AL32UTF8`. |
| 12.2 | solo v3.1.x | El v3.2.x no compila (`PLS-00222`). El charset `WE8DEC` de la imagen pierde caracteres no representables (p. ej. `€`); el driver thin ignora `NLS_LANG`. |
...
| `utplsql.coverage.schemes` | `[]` | Esquemas cubiertos (`a_coverage_schemes`). Vacío = usuario de la conexión (o `utplsql.coverageOwner`). |
| `utplsql.coverage.includeObjects` | `[]` | Objetos a incluir en la cobertura, como `OWNER.NAME` (p. ej. `["APP.MI_PKG"]`). Útil para objetos alcanzados solo dinámicamente. |
| `utplsql.coverage.excludeObjects` | `[]` | Objetos a excluir de la cobertura, como `OWNER.NAME` (p. ej. `["UT3.UT_COVERAGE"]`). |
| `utplsql.coverage.includeSchemaExpr` | `""` | Regex de esquemas a incluir en la cobertura (p. ej. `^APP$`). |
| `utplsql.coverage.includeObjectExpr` | `""` | Regex de objetos a incluir en la cobertura. |
| `utplsql.coverage.excludeSchemaExpr` | `""` | Regex de esquemas a excluir de la cobertura. |
| `utplsql.coverage.excludeObjectExpr` | `""` | Regex de objetos a excluir de la cobertura (p. ej. `^UT_` para el framework utPLSQL). |
...
| `utplsql.oracleClientMode` | `thin` | Modo del driver: `thin` (JavaScript puro, sin cliente nativo) o `thick` (usa Oracle Instant Client). Use `thick` solo en bases que requieren NNE (Native Network Encryption); requiere `utplsql.oracleClientLibDir` y recargar la ventana. |
...
| `utplsql.compilationDiagnostics.enabled` | `true` | Muestra los errores de compilación PL/SQL de la base de datos (`ALL_ERRORS`) como subrayados en el editor y en el Problems Panel (origen "utPLSQL Compilation"). |
...
| `utplsql.profiles` | `[]` | Perfiles de conexión Oracle guardados (nombre, connection y overrides de `sourcePath`/`coverageOwner`/etc.) para alternar entre entornos. **Las contraseñas se guardan en el llavero del SO (VS Code SecretStorage), no en la configuración** — el campo `connection` almacena solo `user@//host:port/service`. Los perfiles heredados con contraseña en línea se migran automáticamente en el primer uso. (Full field reference: [wiki](https://github.com/thepaneb/vscode-utplsql/wiki/Configuration)). |
...
| `utplsql.debugger.compileOnDebug` | `false` | Compila el objeto con información de depuración (`ALTER … COMPILE DEBUG PLSQL_OPTIMIZE_LEVEL = 1`) antes de iniciar la sesión de depuración. |
...
| `utplsql.language` | `auto` | Idioma de los mensajes en tiempo de ejecución. `auto` sigue a VSCode (pt, zh-tw/zh-hk, zh, es, ja, de, fr, it, ko, ru, tr, pl, cs, hu, bg, el, id, ro, sr, th, uk, vi, en-gb; si no, en). Cubre los **24 locales**. |
...
10. **utPLSQL: Cancel run** — detiene la ejecución en curso (`Escape` durante la ejecución).
...
| `-- %tags(fast, critical)` | Etiquetas del test; filtra la ejecución con la setting `utplsql.tags` (p. ej. `fast & !integration`) |
...
| `utPLSQL: Cancel run` | Detiene la ejecución en curso | — |
| `utPLSQL: Show utPLSQL info` | Versiones API/DB con opción de copiar | — |
...
| `utPLSQL: Validate setup` | Ejecuta la validación completa de la configuración (conexión, instalación de UT3) y muestra los resultados | — |
...
La extensión siempre incluye **dos** reporters predeterminados:
`ut_documentation_reporter` (stdout) y
`ut_junit_reporter` (resultados → Test Explorer). El
`ut_coverage_cobertura_reporter` se añade **solo al ejecutar con cobertura**.

**Validación dinámica** — antes de ejecutar con cobertura, la extensión consulta
la base de datos vía `TABLE(ut_runner.get_reporters_list())` para verificar que `UT_COVERAGE_COBERTURA_REPORTER`
...
|---|---|---|
| Las suites no aparecen | Problema de conexión | Ejecute `utPLSQL: Validate setup` para el diagnóstico |
...
| Error de compilación sin indicación | Código con error de sintaxis PL/SQL | Mantenga `utplsql.compilationDiagnostics.enabled` activado (predeterminado); los errores de `ALL_ERRORS` aparecen en el Problems Panel tras una ejecución |
| Error de conexión | Cadena mal formada o DB inaccesible | Use `utPLSQL: Validate setup` |
...
| Necesita diagnósticos | No está claro qué hace la extensión internamente | Defina `UTPLSQL_DEBUG=1` antes de iniciar VSCode para obtener logs de diagnóstico opcionales (contexto de fallos de conexión/discovery/cobertura) en la consola del Extension Host |
