...
[[README (extensão)|English]] · [[README.zh-CN|中文(简体)]] · [[README.zh-TW|中文(繁體)]] · [[README.ja|日本語]] · [[README.ko|한국어]] · [[README.es|Español]] · [[README.fr|Français]] · [[README.pt-BR|Português]] · [[README.it|Italiano]] · [[README.ro|Română]] · **Deutsch** · [[README.ru|Русский]] · [[README.pl|Polski]] · [[README.uk|Українська]] · [[README.cs|Čeština]] · [[README.bg|Български]] · [[README.sr|Српски]] · [[README.tr|Türkçe]] · [[README.el|Ελληνικά]] · [[README.hu|Magyar]] · [[README.id|Bahasa Indonesia]] · [[README.vi|Tiếng Việt]] · [[README.th|ไทย]] · [[README.en-GB|English (UK)]]
...
| `utplsql.coverage.schemes` | `[]` | Abgedeckte Schemas (`a_coverage_schemes`). Leer = Verbindungsbenutzer (oder `utplsql.coverageOwner`). |
| `utplsql.coverage.includeObjects` | `[]` | Objekte, die in die Abdeckung aufgenommen werden, als `OWNER.NAME` (z. B. `["APP.MEIN_PKG"]`). Nützlich für nur dynamisch erreichte Objekte. |
| `utplsql.coverage.excludeObjects` | `[]` | Objekte, die von der Abdeckung ausgeschlossen werden, als `OWNER.NAME` (z. B. `["UT3.UT_COVERAGE"]`). |
| `utplsql.coverage.includeSchemaExpr` | `""` | Regex für Schemas, die in die Abdeckung aufgenommen werden (z. B. `^APP$`). |
| `utplsql.coverage.includeObjectExpr` | `""` | Regex für Objekte, die in die Abdeckung aufgenommen werden. |
| `utplsql.coverage.excludeSchemaExpr` | `""` | Regex für Schemas, die von der Abdeckung ausgeschlossen werden. |
| `utplsql.coverage.excludeObjectExpr` | `""` | Regex für Objekte, die von der Abdeckung ausgeschlossen werden (z. B. `^UT_` für das utPLSQL-Framework). |
...
| `utplsql.language` | `auto` | Sprache der Laufzeitmeldungen. `auto` folgt VSCode (pt, zh-tw/zh-hk, zh, es, ja, de, fr, it, ko, ru, tr, pl, cs, hu, bg, el, id, ro, sr, th, uk, vi, en-gb; sonst en). Deckt die **24 Locales** ab. |
...
| `-- %tags(fast, critical)` | Test-Tags; filtern Sie den Lauf mit der Einstellung `utplsql.tags` (z. B. `fast & !integration`) |
...
Die Erweiterung bindet immer **zwei** Standard-Reporter ein:
`ut_documentation_reporter` (stdout) und
`ut_junit_reporter` (Ergebnisse → Test Explorer). Der
`ut_coverage_cobertura_reporter` wird **nur beim Lauf mit Coverage** hinzugefügt.

**Dynamische Validierung** — vor der Ausführung mit Coverage fragt die Erweiterung
die Datenbank über `TABLE(ut_runner.get_reporters_list())` ab, um zu überprüfen, ob `UT_COVERAGE_COBERTURA_REPORTER`
...
| Diagnosen erforderlich | Unklar, was die Erweiterung intern tut | Setzen Sie `UTPLSQL_DEBUG=1` vor dem Start von VSCode für optionale Diagnoseprotokolle (Kontext von Verbindungs-/Ermittlungs-/Coverage-Fehlern) in der Extension-Host-Konsole |
