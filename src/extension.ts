...
export function activate(context: vscode.ExtensionContext) {
  ...
  controller.resolveHandler = async (item) => { if (!item) await refresh(controller); };
  controller.refreshHandler = async () => { await refresh(controller); };

  state.runProfile = controller.createRunProfile('Run', vscode.TestRunProfileKind.Run, (request, token) => executeRun(controller, request, token, false, state), true);
  state.coverageProfile = controller.createRunProfile('Run with Coverage', vscode.TestRunProfileKind.Coverage, (request, token) => executeRun(controller, request, token, true, state), true);
  state.coverageProfile.loadDetailedCoverage = async (_run, fc) => state.getCoverage(fc.uri.toString());
  ...
  context.subscriptions.push(
    ...
    vscode.commands.registerCommand('utplsql.runAll', () => executeRun(controller, new vscode.TestRunRequest(undefined, undefined, state.runProfile), new vscode.CancellationTokenSource().token, false, state)),
...
async function refresh(controller: vscode.TestController): Promise<void> {
  ...
  for (const suite of suites) {
    const suiteItem = controller.createTestItem(`suite:${suite.packageName.toLowerCase()}`, `${suite.suiteDescription}  (${suite.packageName})`, suite.uri);
    state.setMeta(suiteItem, { kind: 'suite', packageName: suite.packageName, uri: suite.uri });
    for (const t of suite.tests) {
      const testItem = controller.createTestItem(`test:${suite.packageName.toLowerCase()}.${t.procName.toLowerCase()}`, t.description, suite.uri);
      testItem.range = new vscode.Range(t.line, 0, t.line, 0);
      state.setMeta(testItem, { kind: 'test', packageName: suite.packageName, procName: t.procName, description: t.description, uri: suite.uri });
...
function collectAllItems(controller: vscode.TestController): vscode.TestItem[] {
  ...
  controller.items.forEach((i) => state.cachedItems.push(i));
...
async function runForUri(controller: vscode.TestController, uri: vscode.Uri, coverage: boolean) {
  ...
  if (!include.length) { vscode.window.showWarningMessage('Nenhuma suite utPLSQL encontrada neste arquivo.'); return; }
  await executeRun(controller, new vscode.TestRunRequest(include, undefined, coverage ? state.coverageProfile : state.runProfile), new vscode.CancellationTokenSource().token, coverage, state);
}
...
async function runForFolder(controller: vscode.TestController, uri: vscode.Uri, coverage: boolean) {
  ...
  if (!include.length) { vscode.window.showWarningMessage('Nenhuma suite utPLSQL encontrada nesta pasta.'); return; }
  await executeRun(controller, new vscode.TestRunRequest(include, undefined, coverage ? state.coverageProfile : state.runProfile), new vscode.CancellationTokenSource().token, coverage, state);
}
