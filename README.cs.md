...
- 🔧 **Diagnostika nastavení** — proaktivní ověření CLI, připojení, grantů a verze s rychlou opravou (quick-fix).
...
- **Pro režim CLI:** [**utPLSQL-cli**](https://github.com/utPLSQL/utPLSQL-cli/releases) + **Java** nainstalované na počítači (rozšíření volá CLI).
- **Pro režim Oracle direct:** pouze databáze — VSIX už obsahuje tenký ovladač `oracledb` (bez Instant Client).
- **VSCode 1.88+** (Test Coverage API).

Rozšíření je pouze „grafický klient" — to, co testy spouští, je databáze: přes
CLI (utPLSQL-cli + Java) nebo přímo (node-oracledb, `runnerMode: auto` ve výchozím stavu).
...
Profily připojení (`utplsql.profiles`) mohou také přebíjet `sourcePath`, `coverageOwner`, `invocation`, `cliPath` atd. podle prostředí — viz `utplsql.activeProfile` v tabulce konfigurace.
...
Jsou k dispozici dva režimy spouštění:

![Execution architecture — two modes](docs/wiki/images/diagram-arquitetura.png)

### Režim Oracle direct (v0.9.0) — `runnerMode: auto` nebo `oracle`
...
### Režim CLI — `runnerMode: cli` (fallback)

![CLI mode — batch](docs/wiki/images/diagram-cli.png)

Rozšíření sestaví příkazový řádek CLI nebo se připojí přes Oracle direct, přečte
sestavy (JUnit + Coverage) a převede je do nativních API VSCode. Režim
`auto` (výchozí) zkusí Oracle direct a pokud `node-oracledb` není
nainstalovaný, spadne na CLI. Pro vždy vynucení CLI použijte `runnerMode: cli`.
...
| `utplsql.cliPath` | `utplsql` | Cesta ke spustitelnému souboru utPLSQL-cli (např. `C:\tools\utPLSQL-cli\bin\utplsql.bat`). |
...
| `utplsql.extraRunArgs` | `[]` | Další argumenty pro `utplsql run`. |
| `utplsql.coverageOwner` | `""` | Vlastník schématu pokrytých objektů. Prázdné = použije uživatele připojení (velkými písmeny). |
| `utplsql.coverageSourceArgs` | (viz **Coverage**) | Argumenty CLI, které mapují pokrytí na zdrojové soubory. |
| `utplsql.invocation` | `launcher` | Jak volat CLI: `launcher` (přes `.bat`/skript, výchozí) nebo `java` (přímé JVM, **bez shellu**). Viz **Režim volání**. |
| `utplsql.javaPath` | `java` | Spustitelný soubor Java (PATH nebo plná cesta). Používá se pouze v režimu `java`. |
| `utplsql.cliHome` | `""` | Kořen utPLSQL-cli (složka s `bin/` a `lib/`). Prázdné = odvozeno z `cliPath`. Používá se pouze v režimu `java`. |
| `utplsql.timeoutMinutes` | `60` | Časový limit v minutách pro CLI. Příznak `-t` se odesílá pouze v případě, že se hodnota liší od `60`. |
| `utplsql.dbmsOutput` | `false` | Povolí `DBMS_OUTPUT` v testovací relaci. Příznak `-D` se odesílá pouze při `true`. |
| `utplsql.quiet` | `false` | Potlačí informativní logy CLI. Příznak `-q` se odesílá pouze při `true`. |
| `utplsql.failureExitCode` | `1` | Kód návratu při selhání. Příznak `--failure-exit-code` se odesílá pouze v případě, že se hodnota liší od `1`. `0` zajistí, že CLI vždy skončí úspěšně. |
...
| `utplsql.runnerMode` | `auto` | Režim spouštění: `auto` (Oracle direct přes node-oracledb, fallback CLI), `cli` (vždy přes příkazový řádek), `oracle` (vždy Oracle direct). |
...
| `utplsql.javaArgs` | `["-Xmx256m"]` | JVM příznaky pro režim `java` (např. `["-Xmx512m", "-Xms128m"]`). Vloženo před `-cp`. |
| `utplsql.organization` | `file` | Uspořádání stromu: `file` (podle cesty) nebo `schema` (Schema > Package > Suite > Test). V režimu `schema` s Oracle `runnerMode` (`auto`/`oracle`) se sady také objevují z databáze (`ALL_OBJECTS`/`ALL_SOURCE`), když soubory `.pks` nejsou v pracovním prostoru — s virtuální URI `utplsql-db:/` (bez CodeLens/dekorací/skoku na selhání). |
...
| `utplsql.compilationDiagnostics.enabled` | `true` | Zobrazuje chyby kompilace PL/SQL jako podtržení v editoru a v panelu Problémy (režim CLI). |
| `utplsql.setupDiagnostics.enabled` | `true` | Zobrazuje diagnostiku konfigurace (CLI, připojení, granty, verze) a **integritu instalace utPLSQL** (neplatné objekty ve schématu UT3, s rychlou opravou „Recompile UT3") s akcemi rychlé opravy. |
| `utplsql.profiles` | `[]` | Uložené profily připojení k Oracle (název, připojení a přebití `sourcePath`/`coverageOwner`/`invocation`/`cliPath`/atd.) pro přepínání mezi prostředími. |
...
### Režim volání (`launcher` vs `java`)

Ve výchozím nastavení (`utplsql.invocation = "launcher"`) rozšíření volá
launcher `utplsql`/`utplsql.bat`. Na Windows to prochází přes `cmd`, které
**spotřebovává/interpretuje metaznaky** (`^` se stane escape, `|` se stane pipe) — což
rozbíjí regex v `coverageSourceArgs`.

Režim `java` volá JVM **přímo** (`java -cp <home>/etc;<home>/lib/* …
org.utplsql.cli.Cli`), **bez shellu**. Argumenty jdou do procesu jako pole,
bez `cmd` mezi nimi, takže `^` a `|` procházejí **doslovně** — můžete použít `^anchors$` a
`(a|b|c)` v regexu bez obcházení.
...
> Režim `java` věrně replikuje to, co dělá `.bat` (stejný classpath a stejné
> vlastnosti `-D`); jediný rozdíl je, že neprochází přes `cmd`. Vyžaduje `java` na PATH
> (nebo v `utplsql.javaPath`) a to, že kořen CLI je řešitelný — buď přes `cliPath`
> ukazující na `…/bin/utplsql(.bat)`, nebo nastavením `cliHome`.
...
8. **Pro Oracle direct (streaming):** není co instalovat — VSIX už obsahuje tenký ovladač `oracledb`. Režim `auto` spadne na CLI, pokud Oracle není dostupný.
9. Pro diagnostiku použijte `utPLSQL: Show information` v paletě — zobrazí verze CLI/API/DB s možností kopírování.
...
| `utPLSQL: Cancel execution` | Zastaví běžící CLI | — |
| `utPLSQL: Show utPLSQL information` | Verze CLI/API/DB s možností kopírování | — |
...
| `utPLSQL: Validate configuration` | Spustí úplné ověření nastavení (CLI, Java, připojení, instalace UT3) a zobrazí výsledky | — |
...
- **Windows / metaznaky regexu:** v režimu `launcher` (výchozí) prochází `.bat` přes `cmd`,
  které **spotřebovává `^`** a **interpretuje `|` jako pipe** — proto příklady výše používají `\w` a
  `[/\\]` (bez `^`), a `|` v příkladu 2 funguje pouze uvnitř rozšíření. **Řešení:** použijte **`utplsql.invocation = "java"`** (viz
  [Režim volání](#režim-volání-launcher-vs-java)) — bez `cmd` mezi nimi procházejí `^` a `|`
  doslovně a regex můžete psát normálně.
- **Windows / `cmd`:** vyhněte se **`^`** v regexu (`cmd` v `.bat` ho spotřebovává) — proto příklady
  používají `\w` a `[/\\]`.
...
|---|---|---|
| Sady se nezobrazují | CLI nebylo nalezeno | Spusťte `utPLSQL: Validate configuration` pro diagnostiku |
...
| Pomalý výkon | Velké sady vyžadují více paměti JVM heap | Zvyšte `utplsql.javaArgs` (např. `["-Xmx1024m"]`) |
...
| Časový limit při běhu | Testy trvají déle než `timeoutMinutes` | Zvyšte `utplsql.timeoutMinutes` |
| Regex pokrytí se neshoduje | Windows `cmd` spotřebovává `^` a `\|` | Použijte `utplsql.invocation: "java"` (viz [Režim volání](#režim-volání-launcher-vs-java)) |
...
| „report not generated" | CLI nemohlo vygenerovat výstupní XML | Zkontrolujte oprávnění k zápisu v `%TEMP%` a granty utPLSQL |
